...
'''
To run:
    python3 examples/PlotParticles.py --dir optimize.hlt_pixel_optimization_20250127.165402
'''
...
if __name__ == "__main__" :
    ...
    if len(eta_bins) > 0 or len(pt_bins) > 0:
        ...
        for i in np.arange(0, n_agents, max_n_agents):
            ...
            for p_id in range(start_from,start_from+max_n_agents):
                if not p_id in df.index: continue
                efficiencies = 1 - df[df.index == p_id][f"{metric_1}"] # 1MinusEfficiency
                fake_rates = df[df.index == p_id][f"{metric_2}"] # FakeDuplicateRate
                ax.scatter(efficiencies, fake_rates, color=cmap((p_id-start_from)/max_n_agents), 
                        linestyle='--', marker='o', label=f'Part {p_id}')
            # Labels and legend
            title_size = 12
